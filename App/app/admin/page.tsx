...
export default function AdminDashboard() {
  ...
  async function fetchAllData() {
    try {
      ...
      const [clientsRes, companiesRes, ticketsRes] = await Promise.all([
        ...
        supabase.from('company_profiles').select('*'), // Vučemo sve iz profiles
  ...
  async function toggleVerification(id: string, currentStatus: boolean) {
    ...
    try {
      const { error } = await supabase
        .from('company_profiles') // Promijenjeno na company_profiles
          ...
          <div className="hidden md:flex bg-slate-800 p-1 rounded-xl border border-slate-700">
            {(['companies', 'clients', 'tickets'] as TabType[]).map((tab) => (
              <button 
                ...
                className={`px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-yellow-500 text-slate-900' : 'text-slate-400 hover:text-white'}`}
              >
              ...
              <table className="text-left border-collapse table-auto w-full min-w-[600px] md:min-w-0">
                <thead>
                  <tr className="bg-slate-900 text-white">
                    ...
                    <th className="px-3 md:px-8 py-3 md:py-6 text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] text-center">Action / Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  ...
                  ) : (activeTab === 'tickets' ? tickets : activeTab === 'clients' ? clients : companies).map((item, index) => (
                          ...
                          {activeTab === 'tickets' ? (
                            <div className="flex items-center gap-2">
                                <select 
                                    ...
                                    className={`text-[9px] md:text-[10px] font-black uppercase rounded-lg px-2 py-1 border-2 border-slate-100 bg-white cursor-pointer focus:outline-none focus:border-yellow-500 transition-all ${
